...
def Application_review(request, applicationId):
    ...
    if request.method== 'POST':
        ...
        messages.success(request,"Application Has been submited")
        
        return redirect("MyApplication",applicationId=application.applicationId)
...
def Pending_Application(request ):
    ...
    application = Application.objects.filter(ApplicationStatus="Pending")
    ...
    return render(request,'MyApp/Pending_Application.html', {"application":application})





def MyApplication(request, applicationId):
    # Assuming 'applicationId' is a parameter in the URL and you want to filter by it
    application = get_object_or_404(Application, ApplicationStatus="Pending", ApplicationId=applicationId)

    return render(request, 'MyApp/MyApplication.html', {'application': application})